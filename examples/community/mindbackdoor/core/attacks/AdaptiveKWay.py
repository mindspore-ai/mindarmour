'''
This is the implement of Adaptive K-way [1] in mindspore.
...
class PoisonedDatasetFolder(DatasetFolder):
    def __init__(
        ...
        # divide the poisoned set into regularization and payload
        ...
        # k-way payload, each payload with different target
        num_per_k = len(self.payload_indices) // k
        self.payload_label_map = {}
        for i in range(k):
            start = i * num_per_k
            end = (i+1)*num_per_k if i < k-1 else len(self.payload_indices)
            k_group = self.payload_indices[start:end]
            for idx in k_group:
                self.payload_label_map[idx] = k_targets[i]
        ...
        if self.target_transform is None:
            self.poisoned_test_target_transform = Compose([])
        else:
            self.poisoned_test_target_transform = copy.deepcopy(self.target_transform)
    
        self.y_target = k_targets[0]
        self.poisoned_test_target_transform.transforms.insert(poisoned_target_transform_index, ModifyTarget(self.y_target))
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        ...
        if self.poisoned_rate >= 1.0:
            ...
            target = self.poisoned_test_target_transform(target)
        else:
            ...
            if index in self.regularization_indices or index in self.payload_indices:
                ...
                if index in self.payload_indices:
                    target = self.payload_label_map[index]
...
class AdaptiveKWay(Base):
    """
    Construct poisoned datasets with Adaptive K-way attack.
    ...
    def __init__(
        ...
        super(AdaptiveKWay, self).__init__(
