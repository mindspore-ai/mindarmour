...
class YOLOv3Tiny(nn.Cell):
    ...
    def __init__(self, backbone):
        ...
        self.head1 = nn.Conv2d(
            512, config.out_channel, kernel_size=1, stride=1, has_bias=True
        )
        self.head2 = nn.Conv2d(
            256, config.out_channel, kernel_size=1, stride=1, has_bias=True
        )
...
class DetectionBlock(nn.Cell):
    ...
    def __init__(self, scale, config_db=config, is_training=True):
        ...
        if scale == "s":
            ...
            raise ValueError(
                f"Invalid scale {scale} value for DetectionBlock, expected 's' or 'l'."
            )
    ...
    def construct(self, x, input_shape):
        ...
        grid_x = self.tile(
            self.reshape(grid_x, (1, 1, -1, 1, 1)), (1, grid_size[0], 1, 1, 1)
        )
        grid_y = self.tile(
            self.reshape(grid_y, (1, -1, 1, 1, 1)), (1, 1, grid_size[1], 1, 1)
        )
...
class Iou(nn.Cell):
    ...
    def construct(self, box1, box2):
        ...
        intersect_wh = self.max(
            intersect_maxs - intersect_mins, F.scalar_to_tensor(0.0)
        )
        # P.squeeze: for effiecient slice
        intersect_area = P.Squeeze(-1)(intersect_wh[:, :, :, :, :, 0:1]) * P.Squeeze(
            -1
        )(intersect_wh[:, :, :, :, :, 1:2])
...
class YOLOLossBlock(nn.Cell):
    ...
    def __init__(self, scale, config_yb=None):
        ...
        if scale == "s":
            ...
            raise ValueError(
                f"Invalid scale {scale} value for YOLOLossBlock, expected 's' or 'l'."
            )
        self.anchors = Tensor([self.config.anchor_scales[i] for i in idx], ms.float32)
    ...
    def construct(self, prediction, pred_xy, pred_wh, y_true, gt_box):
        ...
        confidence_loss = self.confidence_loss(
            object_mask, prediction[:, :, :, :, 4:5], ignore_mask
        )
        class_loss = self.class_loss(
            object_mask, prediction[:, :, :, :, 5:], class_probs
        )
...
class Giou(nn.Cell):
    ...
    def construct(self, box_p, box_gt):
        """construct method"""
        box_p_area = (box_p[..., 2:3] - box_p[..., 0:1]) * (
            box_p[..., 3:4] - box_p[..., 1:2]
        )
        box_gt_area = (box_gt[..., 2:3] - box_gt[..., 0:1]) * (
            box_gt[..., 3:4] - box_gt[..., 1:2]
        )
        ...
        iou = self.div(
            self.cast(intersection, ms.float32), self.cast(union, ms.float32)
        )
        res_mid0 = c_area - union
        res_mid1 = self.div(
            self.cast(res_mid0, ms.float32), self.cast(c_area, ms.float32)
        )
        giou = iou - res_mid1
