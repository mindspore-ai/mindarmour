        ...
        for index in range(len(flag[0])):
            ...
            for l in range(num_layers):
                if n in anchor_mask[l]:
                    i = np.floor(true_boxes[t, 0] * grid_shapes[l][1]).astype(
                        "int32"
                    )  # grid_y
                    j = np.floor(true_boxes[t, 1] * grid_shapes[l][0]).astype(
                        "int32"
                    )  # grid_x
...
class PreprocessTrueBox:
    ...
    def __call__(self, anno, input_shape):
        ...
        return (
            anno,
            np.array(bbox_true_1),
            np.array(bbox_true_2),
            np.array(gt_box1),
            np.array(gt_box2),
        )
...
def color_distortion(img, hue, sat, val, device_num):
    """Color distortion."""
    hue = _rand(-hue, hue)
    sat = _rand(1, sat) if _rand() < 0.5 else 1 / _rand(1, sat)
    val = _rand(1, val) if _rand() < 0.5 else 1 / _rand(1, val)
    if device_num != 1:
        cv2.setNumThreads(1)
...
def _correct_bbox_by_candidates(
    ...
    while candidates:
        ...
        if allow_outside_center:
            ...
            t_box = t_box[
                np.logical_and(
                    (t_box[:, 0] + t_box[:, 2]) / 2.0 >= 0.0,
                    (t_box[:, 1] + t_box[:, 3]) / 2.0 >= 0.0,
                )
            ]
            t_box = t_box[
                np.logical_and(
                    (t_box[:, 0] + t_box[:, 2]) / 2.0 <= input_w,
                    (t_box[:, 1] + t_box[:, 3]) / 2.0 <= input_h,
                )
            ]
